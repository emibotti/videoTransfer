	...
	printf("\nRecibio conexion\n");
      ...
      int data_size = MAX_MSG_SIZE;
		...
		while (is_connected){
            ...
            std::string holi = data;
			// puts(holi.substr(0, 1).c_str());
            std::string play = "init";
            if (holi.compare(0, play.length(), play) == 0){
                std::string puerto_str = holi.substr(play.length()+1, received_data_size-2).c_str();
